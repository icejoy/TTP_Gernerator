...
public class TTP_Test
{
	...
	public void Start()
	{
		...
		this.ParagraphList = new ArrayList<ArrayList<String>>(this.parser.get(2).get_Stringresult());
		this.ParagraphList = this.SWC.ParagraphClear(this.ParagraphList);
		// 各類別的相似詞
		this.parser.get(2).parse(null,null);
		this.SimilarWords = new ArrayList<ArrayList<String>>(this.parser.get(2).get_Stringresult());
		this.SimilarWords = this.SWC.ArrayListClear(this.SimilarWords);
//		this.output.get(3).start_ArrayList2String("src/main/java/Input_Data/SimilarWords_List.txt", this.SimilarWords);
		
		// recall and precision
		this.parser.get(3).set_TTPs(ttps);
		this.parser.get(3).parse("TTP", "src/main/java/Input_Data/SimilarWords_List.txt");
		this.output.get(3).start_ArrayList2String("./output/TTP_recall_50.txt", this.parser.get(3).get_Stringresult());
...
//				{
//					// 對每個段落
//					W2V_Result.get(Stix_TTP_count).add(this.word2vec.Start(this.SimilarWords.get(0), list.get(i)));
//					System.gc();
//				}
//			}
//		}
...
//		this.output.get(2).start_Match_output("./output/TTP_W2V_output.csv", W2V_Result);
	}
